...
Three different fixing methods are available, which will give different results.

.. figure:: img/fix_geometry_gap.png
   :align: center

   Before and after fixing gaps in polygon features using create new feature method.
...
.. list-table::
   ...
   * - **Neighbors layer**
     - ``NEIGHBORS``
     - [vector: geometry]
     - Layer with the errors location, generated by the check algorithm.
   * - **Gaps layer**
     - ``GAPS``
     - [vector: polygon]
   ...
   * - **Method**
     ...
     - Method to apply to the gap fixing. Possible values are:
       ...
       * 0 -- ``Add to longest shared edge``:
         adds the gap to the longest shared edge with neighboring features.
   ...
   * - **Field of error id**
     - ``ERROR_ID_IDX``
     - [tablefield: any]
     - Field storing the error ID for each gap, as set in the check algorithm.
   * - **Gaps-filled layer**
     - ``OUTPUT``
     - [vector: polygon]
